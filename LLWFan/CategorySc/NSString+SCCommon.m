...
- (NSString*)formatedNumberWithArrNumber:(NSArray*)blankArr isDelete:(BOOL)isDelete {

    NSString *string =self;

        NSInteger stringLength = string.length;

     if (isDelete)  { // 输入框模式删除操作
     ...
     for(NSInteger j =0; j <= i; j ++) {
                    ...
                    blankMark = blankMark + 1 + [blankArr[j] integerValue];

                }

     if(stringLength == blankMark) {

                    string = [string substringToIndex:stringLength -1];

    return string;

    }else if (stringLength < blankMark) { // 查找的长度已超过字符串长度
