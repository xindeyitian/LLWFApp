...
-(void)archiverUserModel:(THUserModel *)userModel{
   ...
   NSLog(@"_+_+_+_+_%@%@",model.userAccount,model.userID);
...
- (void)archiverUserWithDrawModel:(THUserWithDrawModel *)userWithDrawModel{
    ...
    NSError *error = nil;
    if (@available(iOS 11.0, *)) {
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:userWithDrawModel requiringSecureCoding:NO error:&error];
        if (data == nil || error) {
            NSLog(@"归档失败:%@", error);
            return;
        }
        //更新一下 用户模型
        [UserDefaults setObject:data forKey:@"THUserWithDrawModel"];
        if (data == nil || error) {
            NSLog(@"归档失败:%@", error);
            return;
        }
    } else {
        NSData *data = [NSKeyedArchiver archivedDataWithRootObject:userWithDrawModel];
        if (data == nil || error) {
            NSLog(@"归档失败:%@", error);
            return;
        }
...
- (THUserWithDrawModel *)unarchiverUserWithDrawModel:(NSData *)userModelData{
    ...
    if (userModelData) {
        ...
        if (@available(iOS 11.0, *)) {
            THUserWithDrawModel *model = [NSKeyedUnarchiver unarchivedObjectOfClass:[THUserWithDrawModel class] fromData:userModelData error:&error];
            if (model == nil || error) {
                NSLog(@"解归档失败:%@", error);
                return nil;
            }
            ...
            THUserWithDrawModel *model = [NSKeyedUnarchiver unarchiveObjectWithData:userModelData];
            if (model == nil || error) {
                NSLog(@"解归档失败:%@", error);
                return nil;
            }
...
- (void)setUserWithDrawModel:(THUserWithDrawModel *)userWithDrawModel{
    
    _userWithDrawModel = userWithDrawModel;
    //更新一下 用户模型
    [THUserManagerShareTHUserManager archiverUserWithDrawModel:userWithDrawModel];
    [UserDefaults synchronize];
}
/**
...
//    [[XTAPPService shareAppService].currentViewController.navigationController presentViewController:login animated:YES completion:nil];
