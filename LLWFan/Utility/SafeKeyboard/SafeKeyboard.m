...
const static BOOL randomKeyboard = NO;//是否使用随机键盘
const static BOOL changeWhenAppear = YES;//每次键盘出现都改变随机(随机键盘才有效)

#define kKeyBoardHeight (270)//整个键盘的高度
...
- (void)refresh
{
    ...
    {
        _titleArr = [NSMutableArray arrayWithArray:@[@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"X",@"0",@"D"]];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if (indexPath.item == 9) {
            cell.cellTextLabel.backgroundColor = color_f5;
        }else{
            cell.cellTextLabel.backgroundColor = UIColor.whiteColor;
        }
        return cell;
    }
}
...
-(void)setupCell
{
    ...
    self.cellTextLabel.font = [UIFont systemFontOfSize:24 weight:UIFontWeightMedium];
