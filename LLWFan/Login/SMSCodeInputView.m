...
- (void)updateSubViews {
    ...
    for (NSInteger i = 0; i < _arrayTextFidld.count; i ++) {
        ...
        t.frame = CGRectMake(i * (w + _codeSpace), 0, w, w + 10);
    }
}
...
- (void)textFieldDidChangeValue:(NSNotification *)notification {
    ...
    if (sender.text.length == self.codeCount) {
        if (self.delegate != nil && [self.delegate respondsToSelector:@selector(successInputWithText:)]) {
            [self.delegate successInputWithText:sender.text];
        }
    }
}
